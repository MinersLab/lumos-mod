...
class LumiumConversionRecipeWidget(
    val recipe: LumiumConversionRecipe,
) : WidgetGroup() {
    companion object {
        const val WIDTH = 120
        const val HEIGHT = 20
    ...
    init {
        ...
        val worldFluidWidget =
            TankWidget(worldFluidTank, 4 + 18 + 4 + 20 + 4, 0, 20, 20, false, false)
                .setBackground(TankWidget.FLUID_SLOT_TEXTURE)
                .setIngredientIO(IngredientIO.CATALYST)
        val itemInput: List<Either<List<Pair<TagKey<Item>, Int>>, List<ItemStack>>> =
            recipe.inputItem.values.map {
                if (it is TagValue) {
                    Either.left(listOf(Pair.of(it.tag, it.items.first().count)))
                } else {
                    Either.right(it.items.toList())
                }
            }
        val itemInputWidget =
            SlotWidget(TagOrCycleItemStackTransfer(itemInput), 0, 2, 1, false, false)
                .setBackgroundTexture(SlotWidget.ITEM_SLOT_TEXTURE)
                .setIngredientIO(IngredientIO.INPUT)
        val itemOutputWidget =
            SlotWidget(
                ItemStackHandler(NonNullList.of(ItemStack.EMPTY, recipe.result.copy())),
                0,
                4 + 18 + 4 + 20 + 4 + 20 + 4 + 20 + 4,
                1,
                false,
                false,
            ).setBackgroundTexture(SlotWidget.ITEM_SLOT_TEXTURE)
