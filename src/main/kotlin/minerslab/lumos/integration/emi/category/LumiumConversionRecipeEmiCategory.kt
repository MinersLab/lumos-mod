...
class LumiumConversionRecipeEmiCategory : EmiRecipeCategory(Lumos.id("lumium_conversion"), EmiStack.of(ModFluids.LUMIUM.get().bucket)) {
    ...
    inner class Recipe(private val recipe: RecipeHolder<LumiumConversionRecipe>) : LumosEmiRecipe({ LumiumConversionRecipeWidget(recipe.value) }) {
    ...
    fun register(registry: EmiRegistry) {
        ...
        registry.recipeManager.getAllRecipesFor(ModRecipeTypes.LUMIUM_CONVERSION.get())
            .map(::Recipe)
